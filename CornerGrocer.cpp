// Developer : Cisem Tutkavul
// Date : 06/21/2025
// Description : Application loads item purchase information from a file, then shows how many items were purchased, their frequencies, and a histogram of the frequencies.

#include <iostream>
#include <fstream>
#include <map>
#include <string>
#include <iomanip>
using namespace std;

// Tracking and managing item frequencies from file
class ItemTracker {
private:
    map<string, int> itemFrequency;

    // Load data from input file into the Map
    void loadItemsFromFile(const string& filename) {
        ifstream inputFile(filename);
        string item;

        if (!inputFile) {
            cerr << "Error: Could not open " << filename << endl;
            return;
        }

        while (inputFile >> item) {
            itemFrequency[item]++;
        }

        inputFile.close();
    }

    // Section for writing data to frequency.dat
    void writeFrequencyFile(const string& filename) {
        ofstream outputFile(filename);
        for (const auto& pair : itemFrequency) {
            outputFile << pair.first << " " << pair.second << endl;
        }
        outputFile.close();
    }

public:
    // Loads data and writes backup
    ItemTracker(const string& inputFileName, const string& backupFileName) {
        loadItemsFromFile(inputFileName);
        writeFrequencyFile(backupFileName);
    }

    // This section helping to find specific item
    int getItemFrequency(const string& item) const {
        auto it = itemFrequency.find(item);
        if (it != itemFrequency.end()) {
            return it->second;
        }
        else {
            return 0;
        }
    }

	// This section for printing all items and their frequencies
    void printItemFrequencies() const {
        cout << "\nItem Purchase Frequencies:\n";
        for (const auto& pair : itemFrequency) {
            cout << left << setw(15) << pair.first << pair.second << endl;
        }
    }

    // This section for histogram of item frequencies
    void printHistogram() const {
        cout << "\nItem Frequency Histogram:\n";
        for (const auto& pair : itemFrequency) {
            cout << left << setw(15) << pair.first << string(pair.second, '*') << endl;
        }
    }
};

// Menu options
void printMenu() {
    cout << "\n--- Corner Grocer Item Tracker ---\n"
        << "1. Search for item frequency\n"
        << "2. Print all item frequencies\n"
        << "3. Print item histogram\n"
        << "4. Exit\n"
        << "Choose an option: ";
}

int main() {
    ItemTracker tracker("CS210_Project_Three_Input_File.txt", "frequency.dat");
    int choice;
    string item;

    while (true) {
        printMenu();
        cin >> choice;

		// Input validation
        if (cin.fail()) {
            cin.clear();
            cin.ignore(10000, '\n');
            cout << "Invalid input. Please enter a number from 1 to 4.\n";
            continue;
        }

        switch (choice) {
        case 1:
            cout << "Enter item name: ";
            cin >> item;
            cout << item << " was purchased " << tracker.getItemFrequency(item) << " time(s).\n";
            break;
        case 2:
            tracker.printItemFrequencies();
            break;
        case 3:
            tracker.printHistogram();
            break;
        case 4:
            cout << "Goodbye!\n";
            return 0;
        default:
            cout << "Invalid option. Please select between 1 and 4.\n";
        }
    }

    return 0;
}
